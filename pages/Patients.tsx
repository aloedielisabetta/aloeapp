...
import { 
  ...
  Loader2, Activity, Scale, Clipboard, Save, MessageSquare
} from 'lucide-react';
...
const Patients: React.FC = () => {
  ...
  const handleDownloadProtocol = async (patient: Patient) => {
    ...
    setTimeout(async () => {
      ...
      const opt = {
        margin: 10,
        filename: `Protocollo_Aloe_${patient.firstName}_${patient.lastName}.pdf`,
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-3xl font-black tracking-tight text-slate-900 uppercase">Gestione Pazienti</h2>
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 md:pb-0 w-full md:w-auto scrollbar-hide bg-slate-50/50 p-2 rounded-2xl">
          <button 
            ...
            className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
              selectedCity === 'Tutte' ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'
            }`}
          >
            Tutte
          </button>
          {cities.map(city => (
            <button 
              ...
              className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${
                selectedCity === city.name ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'
              }`}
            >
              {city.name}
            </button>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center py-3 border-y border-slate-50">
                ...
                <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-tighter ${
                  patient.medicalState === 'Critico' ? 'bg-red-50 text-red-600' :
      ...
      {/* HIDDEN PDF TEMPLATE (ENHANCED WITH JOURNAL) */}
      <div className="hidden">
        <div ref={protocolRef} className="p-12 font-sans text-slate-900 bg-white" style={{ minHeight: '297mm', width: '210mm' }}>
           <div className="flex justify-between items-start border-b-8 border-green-600 pb-10 mb-12">
              <div className="flex items-center gap-6">
                 <span className="text-6xl">🌱</span>
                 <div>
                    <h1 className="text-5xl font-black tracking-tighter uppercase mb-1">Laboratorio Aloe</h1>
                    <p className="text-sm font-bold text-slate-400 uppercase tracking-[0.3em]">Protocollo Nutrizionale di Elisabetta</p>
                 </div>
              </div>
              <div className="text-right">
                 <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Documento Riservato</p>
                 <p className="text-lg font-black">{new Date().toLocaleDateString('it-IT')}</p>
              </div>
           </div>

           <div className="grid grid-cols-2 gap-12 mb-12">
              <div className="space-y-6">
                 <h2 className="text-xs font-black uppercase tracking-[0.2em] text-green-600 border-b-2 border-green-50 pb-3">Anagrafica Paziente</h2>
                 <div>
                    <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest mb-1">Paziente</p>
                    <p className="text-3xl font-black uppercase text-slate-800">{activeProtocolPatient?.firstName} {activeProtocolPatient?.lastName}</p>
                 </div>
                 <div className="flex gap-10">
                    <div>
                      <p className="text-[9px] font-black text-slate-300 uppercase">Città</p>
                      <p className="text-sm font-bold">{activeProtocolPatient?.city}</p>
                    </div>
                    <div>
                      <p className="text-[9px] font-black text-slate-300 uppercase">Telefono</p>
                      <p className="text-sm font-bold">{activeProtocolPatient?.phone}</p>
                    </div>
                 </div>
              </div>
              <div className="space-y-6">
                 <h2 className="text-xs font-black uppercase tracking-[0.2em] text-green-600 border-b-2 border-green-50 pb-3">Profilo Clinico</h2>
                 <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-[9px] font-black text-slate-300 uppercase">Patologia</p>
                      <p className="text-sm font-bold">{activeProtocolPatient?.medicalCondition || 'Monitoraggio Benessere'}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-[9px] font-black text-slate-300 uppercase">Stato Attuale</p>
                      <p className="text-sm font-black uppercase text-green-700 bg-green-50 px-2 py-1 rounded inline-block">{activeProtocolPatient?.medicalState}</p>
                    </div>
                 </div>
              </div>
           </div>

           <div className="bg-slate-50 p-12 rounded-[3.5rem] border border-slate-100 mb-12 shadow-inner">
              <h2 className="text-sm font-black uppercase tracking-[0.3em] text-slate-800 mb-8 flex items-center gap-4">
                 <span className="w-10 h-10 bg-green-600 text-white rounded-2xl flex items-center justify-center shadow-lg">01</span>
                 Dettagli Protocollo Aloe
              </h2>
              <div className="prose prose-slate max-w-none">
                 <p className="text-lg text-slate-700 leading-relaxed whitespace-pre-wrap font-medium italic">
                    {activeProtocolPatient?.aloeTweak || 'Si consiglia il dosaggio standard di 3 cucchiai al giorno prima dei pasti.'}
                 </p>
              </div>
           </div>

           {/* History section in PDF */}
           {(activeProtocolPatient?.journal?.length || 0) > 0 && (
             <div className="mb-12">
                <h2 className="text-xs font-black uppercase tracking-[0.3em] text-slate-800 mb-8 flex items-center gap-4">
                  <span className="w-10 h-10 bg-blue-600 text-white rounded-2xl flex items-center justify-center shadow-lg">02</span>
                  Ultimi Aggiornamenti Clinici
                </h2>
                <div className="space-y-6">
                  {activeProtocolPatient?.journal?.slice(0, 3).map((entry, i) => (
                    <div key={i} className="p-6 bg-white border border-slate-100 rounded-3xl">
                       <div className="flex justify-between mb-2">
                          <span className="text-[10px] font-black text-blue-600 uppercase">{new Date(entry.date).toLocaleDateString('it-IT')}</span>
                          {entry.weight && <span className="text-[10px] font-black text-slate-400">PESO: {entry.weight} KG</span>}
                       </div>
                       <p className="text-sm text-slate-600 font-bold mb-2">{entry.healthStatus}</p>
                       <p className="text-[11px] font-black text-emerald-600 uppercase italic">Dosaggio Applicato: {entry.aloeDosage}</p>
                    </div>
                  ))}
                </div>
             </div>
           ...
           <div className="mt-auto pt-24 grid grid-cols-2 items-end">
              <div>
                 <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.3em] mb-1">Identificativo Paziente</p>
                 <p className="text-[9px] font-mono text-slate-400">{activeProtocolPatient?.id.toUpperCase()}</p>
              </div>
              <div className="text-right">
                 <div className="w-48 h-px bg-slate-200 mb-4 ml-auto"></div>
                 <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Firma Elisabetta</p>
                 <p className="text-[9px] text-slate-400 font-bold mt-1">Responsabile Laboratorio</p>
              </div>
           </div>
        </div>
      </div>
          ...
          <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col border border-white/20">
            <div className="p-8 border-b border-slate-50 flex justify-between items-center bg-slate-50/50 shrink-0">
               <div className="flex items-center gap-4">
                  ...
                  <h3 className="text-2xl font-black text-slate-800 uppercase tracking-tight">{editingPatient ? 'Modifica Anagrafica' : 'Nuovo Onboarding'}</h3>
               </div>
